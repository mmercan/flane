...
CREATE FUNCTION [flane].[GetConfirmedSchedules]
(	
)
RETURNS TABLE 
AS
...
SELECT        sch.ID as ServiceActivityID,sch.CityID as SiteID, sch.ProductID, sch.Name as subject, sch.StartDate as ActualStart, 
						 ...
						 cast(1 as bit) as IsShowConfirmed
...
WHERE        (sch.StartDate >= GETDATE()) AND (sch.status = 102880005) and (sch.publish != '102880000' or sch.publish is null)
